...
export default async function PrivacyPolicyPage() {
  ...
  let customPrivacyBody: string | null = null;
  try {
    const { data } = await supabase
      .from('global_settings')
      .select('value')
      .eq('key', 'legal.privacy_policy_body')
      .maybeSingle();

    customPrivacyBody = data?.value ?? null;
  } catch {
    // Fall back to default Privacy Policy on error
  }
        ...
        <div className="max-w-4xl mx-auto relative z-10">
          ...
          <div className="bg-white p-8 sm:p-12 rounded-[var(--radius-3xl)] text-sm text-[var(--color-text-secondary)] space-y-8 leading-relaxed shadow-sm border border-gray-100">
            <EditableLegalBody
              ...
              initialValue={customPrivacyBody}
